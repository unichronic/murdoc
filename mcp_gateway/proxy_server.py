"""
Standalone AgentVault MCP proxy.

This process presents an MCP server to upstream clients and forwards allowed
tool traffic to a configured downstream MCP server through the shared Bifrost
MCP adapter.
"""
...
try:
    from .adapter import BifrostMCPGateway, MCPGatewayContext
except ImportError:
    from adapter import BifrostMCPGateway, MCPGatewayContext
...
class AgentVaultMCPProxy:
    def __init__(
        self,
        *,
        session_factory: SessionFactory,
        adapter: BifrostMCPGateway | None = None,
        ...
        self.adapter = adapter or BifrostMCPGateway()
    ...
    async def __aenter__(self) -> "AgentVaultMCPProxy":
        await self.start()
        return self
...
def create_mcp_proxy_server(proxy: AgentVaultMCPProxy) -> Server:
    server = Server("agentvault-mcp-gateway")
...
async def run_stdio_proxy(proxy: AgentVaultMCPProxy) -> None:
...
def main() -> None:
    ...
    try:
        ...
        print(f"AgentVault MCP proxy configuration error: {exc}", file=sys.stderr)
    ...
    proxy = AgentVaultMCPProxy(session_factory=factory, server_id=os.getenv("MCP_SERVER_ID", "default"))
