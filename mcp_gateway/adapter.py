"""
Shared MCP adapter for the Bifrost runtime.

The adapter is the protocol boundary between MCP sessions and the shared
Bifrost security runtime. It is intentionally independent from any specific MCP
...
logger = logging.getLogger("agentvault.mcp-adapter")


class MCPSecurityViolation(Exception):
    """Raised when an MCP request or response is blocked by AgentVault."""

    def __init__(self, layer: str, reason: str, tool_name: str):
        self.layer = layer
        self.reason = reason
        self.tool_name = tool_name
        super().__init__(f"[AgentVault/{layer}] Blocked tool '{tool_name}': {reason}")
...
class BifrostMCPGateway:
    def __init__(
        self,
        *,
        bifrost: BifrostGateway | None = None,
        tool_config: MCPToolSecurityConfig = CONFIG,
        tool_allowed: ToolAllowedFn | None = None,
    ):
        self.bifrost = bifrost or BifrostGateway()
    ...
    async def authorize_tool_call(
        ...
        outcome = await self.bifrost.authorize_tool_call(
            tool_name,
            tool_args,
        ...
        if outcome.blocked:
            raise MCPSecurityViolation(
                layer=outcome.blocked_layer or "bifrost",
    ...
    async def inspect_tool_result(
        ...
        if hasattr(self.bifrost, "inspect_tool_result"):
            outcome = await self.bifrost.inspect_tool_result(
                tool_name,
                result_text,
        ...
        scrubbed = await self.bifrost.scrub_tool_result(tool_name, result_text)
