...
def __getattr__(name):
    ...
    if name == "scan_prompt":
        ...
        return scan_prompt
    if name == "scan_output":
        from murdoc.security.presidio_scanner import scan_output
        return scan_output
    if name == "redact_output":
        from murdoc.security.presidio_scanner import redact_output
        return redact_output
    if name == "scan_semantics":
        from murdoc.security.semantic_guardrails import scan_semantics
        return scan_semantics
    raise AttributeError(f"module 'murdoc.security' has no attribute {name!r}")
