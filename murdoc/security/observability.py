...
SERVICE_NAME = "murdoc-gateway"
logger = logging.getLogger("murdoc.observability")
DEFAULT_EVENT_LOG_FILE = "logs/murdoc-events.jsonl"
...
class Observability:
    def __init__(self, app=None):
        ...
        self.actual_usage_cost = Counter(
            "murdoc_usage_actual_cost_usd_total",
        ...
        self.pii_entities = Counter(
            "murdoc_pii_entities_detected_total",
    ...
    def histogram_percentiles(self) -> dict[str, dict[str, dict[str, float | int | str]]]:
        return {
            "layer": self._histogram_summary("murdoc_security_layer_duration_seconds", "layer"),
            "tier": self._histogram_summary("murdoc_security_tier_duration_seconds", "tier"),
            "http": self._histogram_summary("murdoc_http_request_duration_seconds", "route"),
        }
    ...
    def layer(self, name, reason="none"):
        ...
        decision = "error"
        with self.span(f"murdoc.security.{name}", security_layer=name):
            try:
    ...
    def _set_decision(self, layer, decision, reason, started):
        ...
        self.security_event(
            "murdoc.security.layer.completed",
            layer=layer,
            decision=decision,
            reason=safe_reason,
        )
