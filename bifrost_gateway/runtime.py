...
and MCP tool interception use the same Bifrost request evaluation flow.
"""
...
from security.config import (
    BIFROST_READ_CACHE_ENABLED,
    BIFROST_READ_CACHE_MAX_ITEMS,
    BIFROST_READ_CACHE_TTL_SECONDS,
    LAKERA_API_KEY,
    LAKERA_REQUIRED,
...
from security.observability import prompt_fingerprint
...
from security.presidio_scanner import PresidioResult, async_redact_output, async_scan_output
from security.semantic_guardrails import SemanticGuardrailResult, scan_semantics
...
class BifrostGateway:
    ...
    def _cacheable_intent(
        ...
        if not BIFROST_READ_CACHE_ENABLED:
            return False
    ...
    def _get_cached_response(self, cache_key: str) -> dict[str, Any] | None:
        ...
        if entry is None:
            return None
        if time.monotonic() - entry.created_at > BIFROST_READ_CACHE_TTL_SECONDS:
    ...
    def _store_cached_response(self, cache_key: str, payload: dict[str, Any]) -> None:
        ...
        self.response_cache.move_to_end(cache_key)
        while len(self.response_cache) > BIFROST_READ_CACHE_MAX_ITEMS:
    ...
    async def _preflight(
        ...
        *,
        ...
        if not text:
            ...
            return BifrostProcessOutcome(400, result, blocked_layer="gateway", blocked_reason="empty_request")
        ...
        if lakera_result.error and settings.lakera_required and lakera_mode == "enforce":
            ...
            return BifrostProcessOutcome(503, result, blocked_layer="lakera", blocked_reason="lakera_unavailable")
        ...
        if self._cacheable_intent(normalized_text, contexts, auth, route_profile):
            ...
            with _layer(self.obs, "cache") as record_cache:
                ...
                if cached_payload is not None:
                    ...
                    outcome = BifrostProcessOutcome(200, cached_payload)
    ...
    async def evaluate_payload(
        ...
        blocked_by = None
        if blocked:
            layer = preflight.blocked_layer or "bifrost"
            if layer == "lakera":
