...
async def run_agent(user_query: str) -> str:
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            print(f"\nDiscovered {len(discovered_tools)} Notion MCP tools; exposed {len(mcp_tools)} after policy filtering:")
            for t in mcp_tools:
                print(f"   - {t.name}")
            if hidden_count:
            ...
            while True:
                ...
                for tool_call in assistant_message.tool_calls:
                    ...
                    try:
                        tool_result = await secure_call_tool(
                            session,
                            fn_name,
                            fn_args,
                            context={
                                "client": "notion_mcp_client",
                                "server_id": "notion",
...
async def main():
    print("=" * 60)
    print("  Notion MCP Client - AI to Notion via MCP")
    print("=" * 60)
